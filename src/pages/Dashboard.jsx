...
const EMPTY_FORM = {
  name: "", brand: "", category: "Móviles", price: "",
  stock: "", condition: "Nuevo", description: "",
};
    ...
    >
      ...
      {preview ? (
        ...
        <>
          ...
          <p className="text-zinc-700 text-xs mt-1">PNG, JPG, WEBP — max 5 MB</p>
        </>
      )}
    </div>
  );
...
export default function Dashboard() {
  ...
  const handleEdit = (product) => {
    ...
    setForm({
      name: product.name || "",
      brand: product.brand || "",
      category: product.category || "Móviles",
      price: product.price || "",
      stock: product.stock || "",
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            value={`$${totalValue.toLocaleString("es-DO")}`}
            color="bg-amber-700"
          />
        </section>
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
            <h2 className="font-semibold text-white flex items-center gap-2">
              {editingId ? <><Pencil size={16} className="text-violet-400" /> Editar producto</> : <><Plus size={16} className="text-violet-400" /> Nuevo producto</>}
            </h2>
            {editingId && (
          ...
          <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-5">
              ...
              <FormField label="Descripción (opcional)">
                <textarea
                  rows={3}
                  placeholder="Características, accesorios incluidos, estado del equipo..."
                  ...
                  <div className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-1.5">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${form.condition === "Nuevo" ? "bg-emerald-900/60 text-emerald-400" : "bg-amber-900/60 text-amber-400"}`}>
                          {form.condition}
                        </span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${Number(form.stock) > 0 ? "bg-blue-900/60 text-blue-400" : "bg-red-900/60 text-red-400"}`}>
                          {Number(form.stock) > 0 ? `${form.stock} en stock` : "Sin stock"}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <tr key={p.id} className="hover:bg-zinc-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                          Number(p.stock) > 0 ? "bg-emerald-900/50 text-emerald-400" : "bg-red-900/50 text-red-400"
                        }`}>
                          {p.stock}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-center hidden md:table-cell">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          p.condition === "Nuevo" ? "bg-blue-900/50 text-blue-400" : "bg-amber-900/50 text-amber-400"
                        }`}>
                          {p.condition}
                        </span>
                      </td>
                          ...
                          {deleteConfirm === p.id ? (
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                ...
                                className="p-1.5 text-zinc-400 hover:bg-zinc-800 rounded-lg transition-all"
                                title="Cancelar"
                              >
                                <X size={14} />
                              </button>
                            ...
                            <button
                              ...
                              className="p-1.5 text-zinc-400 hover:text-red-400 hover:bg-red-900/30 rounded-lg transition-all"
                              title="Eliminar"
                            >
                              <Trash2 size={14} />
                            </button>
